# coding: utf-8

"""
    Visier Data Intake APIs

    Visier APIs for sending raw or untransformed source data to Visier

    The version of the OpenAPI document: 22222222.99201.1200
    Generated by: https://konfigthis.com
"""

import unittest
from unittest.mock import patch

import urllib3

import visier_data_intake_python_sdk
from visier_data_intake_python_sdk.paths.v1_op_data_sources import get
from visier_data_intake_python_sdk import configuration, schemas, api_client

from .. import ApiTestMixin


class TestV1OpDataSources(ApiTestMixin, unittest.TestCase):
    """
    V1OpDataSources unit test stubs
        Retrieve a list of sources
    """

    def setUp(self):
        pass

    def tearDown(self):
        pass

    response_status = 200




if __name__ == '__main__':
    unittest.main()
